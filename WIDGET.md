...
| `hosts` | 否 | `net` 权限的域名白名单(如 `["api.open-meteo.com"]`),代理只放行这些;`["*"]` 表示任意域名(RSS 阅读器这类目标由用户填的组件),装的时候用户看得见 |
...
绝不往用户看得见的文件夹里塞 `widgets/`。文件面板的树顶是主目录,`.mainbench` 这类点开头的顶层目录会被过滤掉,用户不会在树里撞见它。

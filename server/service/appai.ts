// 应用的 ai 能力:无状态单次补全 —— 不建智能体、不进邮箱、没有历史。
// 每次调用落一条活动(activities),用户在活动面板看到 summary 与用量;summary 必填。
// 用用户在设置里配好的同一套模型;每应用并发限 2,可见性是第一道闸。
...
export const runAppAi = async ({
  ...
  const current = inflight.get(appId) || 0;
  if (current >= MAX_CONCURRENT) throw new Error("该应用的 AI 调用过于频繁,请稍后再试");

  const activityId = startActivity(`app:${appId}`, "ai", String(summary).slice(0, 200));
  inflight.set(appId, current + 1);
  try {
    ...
    inflight.set(appId, (inflight.get(appId) || 1) - 1);
  }
};

...
var WebSocketTransport = class {
  constructor(webSocket) {
    ...
    webSocket.addEventListener("message", (event) => {
      if (this.#error) {
      } else if (typeof event.data === "string" || event.data instanceof ArrayBuffer) if (this.#receiveResolver) {
...
// overseer-src.mjs
var facade = (stub) => new Proxy(stub, {
  get(target, prop) {
...
var HostGate = class extends WorkerEntrypoint {
  async dbEcho(sql) {
    return `gate(app=${this.ctx.props?.appId}) got: ${sql}`;
  }
};
var APP_CODE = `
import { WorkerEntrypoint } from "cloudflare:workers";
...
var loadGadget = (env, ctx) => env.LOADER.get("app-counter2", () => ({

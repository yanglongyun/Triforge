...
export function ProjectList() {
  ...
  async function submitCreate() {
    ...
    try {
      ...
      await api.generate(project.id, value, count, project.nodeIds);
    } catch (error) {
      setSubmitError(error instanceof Error && error.message ? error.message : t('gen.unavailable'));
    } finally {
      setCreating(false);
    }
  }
  ...
  return (
    <div className="bd-home">
      ...
      <main className="bd-board">
        <HomeComposer prompt={prompt} promptRef={promptRef} plant={plant} count={count}

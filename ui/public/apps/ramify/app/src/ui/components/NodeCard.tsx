...
export const NodeCard = memo(function NodeCard({
  ...
  if (isTitleOnly) {
    return (
      ...
      >
        {onBranch && <button className="cb-node-branch" title={isRoot ? t('gen.branchRoot') : t('gen.branchNode', { title: node.title })} onClick={(e) => { e.stopPropagation(); onBranch(node.id); }}><BranchIcon /></button>}
        {hasChildren && (
  ...
  if (node.type === 'text' && node.content) {
    return (
      <div
        ...
        onClick={(e) => { e.stopPropagation(); onClick(node.id); }}
      >
        {onBranch && <button className="cb-node-branch" title={t('gen.branchNode', { title: node.title })} onClick={(e) => { e.stopPropagation(); onBranch(node.id); }}><BranchIcon /></button>}
        {hasChildren && (
          <button className={`cb-node-collapse${collapsed ? ' is-collapsed' : ''}`} title={collapsed ? t('node.expand', { count: hiddenCount }) : t('node.collapse')} onClick={(e) => { e.stopPropagation(); onToggle(node.id); }}>{collapsed ? hiddenCount : '−'}</button>
        )}
  ...
  return (
    ...
    >
      {onBranch && hasArtifact && (
        <button className="cb-node-branch" title={t('gen.branchNode', { title: node.title })} onClick={(e) => { e.stopPropagation(); onBranch(node.id); }}>
          <BranchIcon />
        </button>
      )}
      {hasChildren && (

...
export function ChatBubble({ node, pos, onSubmit, onClose }: {
  ...
  return (
    <div className="cb-branch-panel" style={{ ...bubblePos, width: 280, ['--arrow-top' as string]: `${arrowTop}px` }}
      ...
      <div className="cb-branch-head">{isRoot ? t('gen.branchRoot') : t('gen.branchNode', { title: node.title })}</div>
      <textarea ref={ref} value={text} onChange={(e) => setText(e.target.value)}
        placeholder={t('gen.branchPlaceholder')}
        onKeyDown={(e) => {
          if (e.key === 'Enter' && !e.shiftKey && !e.nativeEvent.isComposing) { e.preventDefault(); submit(); }
          if (e.key === 'Escape') onClose();
        }} />
      <div className="cb-branch-row"><CountPicker value={count} onChange={setCount} /></div>
      <button className="cb-button cb-button-primary cb-branch-submit" onClick={submit}>{t('gen.branch')} · {count}</button>
    </div>
  );
}
